...
price_filter = 5
signal_name = "smoothed_bma_reversal"
signal_name_title = "Smoothed BMA Reversal"
IC = 0.05
gamma = 140
n_cpus = 8
...
decay_factor = 0.90 
ema_alpha = 0.50 
checkpoint_dir = Path("temp/checkpoints_smoothed_bma")
...
# Weighted regression with normalization for criteria consistency
...
# Walk forward loop with exponential moving average state recovery
rolling_results = []
prev_forecasts = {f: 0.0 for f in reversal_factors}

# Prime the moving average state from the most recent checkpoint if available
...
for i in range(dynamic_window_months, len(unique_dates)):
    ...
    if os.path.exists(checkpoint_path):
        ...
        # Continuous state update to ensure moving average chain is unbroken
        for f in reversal_factors: 
    ...
    # Apply recency decay to training window observations
    weight_map = {
        d: (decay_factor ** (len(train_dates) - 1 - idx)) 
        for idx, d in enumerate(train_dates)
    }
    ...
    for target_factor in reversal_factors:
        ...
        bics, params = get_bics_weighted(df_train, y_dyn, all_combos)
        
        # Calculate posterior model probabilities
        probs = np.exp(-0.5 * (bics - np.min(bics)))
        probs /= probs.sum()
        ...
        # Smoothing layer to mitigate high frequency turnover
...
# Forward returns calculation on original continuous dataset
forward_returns = (
