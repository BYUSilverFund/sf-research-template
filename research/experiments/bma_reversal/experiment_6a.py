...
price_filter = 5
signal_name = "winsorized_volume_bma"
signal_name_title = "Winsorized Log-Volume BMA"
IC = 0.05
gamma = 140  
n_cpus = 8
constraints = [
...
checkpoint_dir = Path("temp/checkpoints_winsor")

# Volume inhibition parameters
vol_window = 20
vol_threshold = 1.1  
vol_slope = 2.0
...
# Load asset data including daily volume
returns = (
    sfd.load_assets(
        start=start,
        end=end,
...
# Compute raw features and log relative volume metrics
data = (
    returns
    .sort("date", "barrid")
    .with_columns(
...
winsor_cols = ["return"] + reversal_factors
...
data = (
    ...
    .with_columns(
        [
            pl.col(c)
            .clip(
                lower_bound=pl.col(c).quantile(0.01).over("date"),
                upper_bound=pl.col(c).quantile(0.99).over("date")
            )
            .alias(c)
...
# Cross sectional score and invert (Using Winsorized Data)
factors = (
    data
    .select(
...
# Lag signals and volume metrics to prevent lookahead bias
data = (
    data
    .with_columns(
        [
            ...
            .shift(1)
            .over("barrid")
            .alias("rel_vol_log_lag")
        ]
    )
    .drop_nulls(
        subset=[f"{f}_lag" for f in reversal_factors] + ["rel_vol_log_lag"]
    )
)

# Compute daily factor returns (Using Winsorized Returns)
...
for k in range(1, len(reversal_factors) + 1):
    all_combos.extend(
        list(combinations(reversal_factors, k))
    )

# Calculate static shrinkage priors once outside the loop
...
existing_checkpoints = sorted(
    list(checkpoint_dir.glob("*.parquet"))
)
...
for i in range(prior_window_months, len(unique_dates)):
    ...
    for target_factor in reversal_factors:
        ...
        smoothed = (
            (ema_alpha * expected_return) + 
            ((1 - ema_alpha) * prev_forecasts[target_factor])
        )
    ...
    .select(
        [
            ...
            (
                pl.col(signal_name)
                .sub(
                    pl.col(signal_name).mean().over("date")
                )
                .truediv(
                ...
                .mul(
                    ...
                    .truediv(
                        ...
                        .add(
                            (
                                pl.col("rel_vol_log_lag")
                                .fill_null(0.0)
                                .clip(lower_bound=0.0)
                                .truediv(vol_threshold)
                            )
                            .pow(vol_slope)
                        )
                    )
...
# Forward returns calculation on continuous dataset with safety fill
forward_returns = (
    returns
    .sort("date", "barrid")
    .select(
