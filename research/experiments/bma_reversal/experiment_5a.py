...
price_filter = 5
signal_name = "volume_inhibited_bma"
signal_name_title = "Volume Inhibited BMA"
IC = 0.05
gamma = 140  
n_cpus = 8
...
checkpoint_dir = Path("temp/checkpoints_volume")
...
# Compute features and log relative volume metrics
data = returns.sort("date", "barrid").with_columns(
    [
        ...
        .alias("prev_price"),
        (
            pl.col("daily_volume").add(1).log().sub(
                pl.col("daily_volume").add(1).log()
                .sort_by("date")
...
for idx, enumerate_subset in enumerate(all_combos):
    if len(enumerate_subset) == 0:
