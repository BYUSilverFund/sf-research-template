...
price_filter = 5
signal_name = "double_sort_log_amihud_bma"
signal_name_title = "Double Sort (MCap + Log Amihud) BMA"
IC = 0.05
gamma = 115
n_cpus = 8
constraints = ["ZeroBeta", "ZeroInvestment"]
results_folder = Path("results/conditional_bma_reversal/experiment_6")

# BMA parameters (Pure Conditional, no priors or vol-adjustments)
dynamic_window_months = 12
checkpoint_dir = "temp/checkpoints_double_sort_log_amihud_bma"
...
data = returns.sort("date", "barrid").with_columns([
    ...
    # Calculate Daily Log-Amihud to compress outliers
    (
        pl.col("return").abs().add(1e-6).log() 
        - (pl.col("price").mul(pl.col("daily_volume")).add(1)).log()
    ).alias("daily_log_amihud")
])

# Smooth Log-Amihud with 20-day rolling median
data = data.with_columns([
    pl.col("daily_log_amihud").sort_by("date").rolling_median(window_size=20).over("barrid").alias("amihud_illiquidity")
])
...
scored_factors = (
    tradable_universe.drop_nulls(["amihud_illiquidity", "market_cap"])
    .with_columns([
        # First Sort: Market Cap Quintiles over the whole date cross-section
        pl.col("market_cap")
    ...
    .with_columns([
        # Second Sort: Log-Amihud Quintiles within each Market Cap Quintile
        pl.col("amihud_illiquidity")
        .qcut(5, labels=["1_Liq", "2_SLiq", "3_Mid", "4_Illiq", "5_VIlliq"], allow_duplicates=True)
        .over(["date", "mcap_q"]) 
        .cast(pl.String)
        .alias("amihud_q")
    ])
    .with_columns([
        # Combine to create 25 unique buckets safely
        pl.concat_str([pl.col("mcap_q"), pl.lit("_"), pl.col("amihud_q")]).alias("size_bucket")
    ])
    .select(["date", "barrid", "size_bucket"] + reversal_factors)
    .drop_nulls()
    .with_columns([
        # Z-Score factors cross-sectionally
...
sfp.generate_ic_chart(
    ics=ics, 
    title=f"{signal_name_title} Cumulative Rank IC", 
    ic_type="Rank", 
    file_name=results_folder / "rank_ic_chart.png"
)
sfp.generate_ic_chart(
    ics=ics, 
    title=f"{signal_name_title} Cumulative Pearson IC", 
    ic_type="Pearson", 
