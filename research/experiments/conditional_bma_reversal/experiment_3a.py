...
price_filter = 5
signal_name = "vol_adjusted_bma_reversal"
signal_name_title = "Vol-Adjusted Hierarchical BMA Reversal"
IC = 0.05
gamma = 115 
n_cpus = 8
constraints = ["ZeroBeta", "ZeroInvestment"]
results_folder = Path("results/conditional_bma_reversal/experiment_3")

# BMA parameters
dynamic_window_months = 12
checkpoint_dir = "temp/checkpoints_vol_adjusted_bma"
...
m_fac_rets = (
    ...
    .group_by_dynamic("date", every="1mo", by="size_bucket")
    .agg([
...
# Calculate rolling 12-month standard deviation of factor returns to act as the denominator
m_fac_rets = m_fac_rets.sort("date", "size_bucket").with_columns([
    pl.col(f).rolling_std(window_size=12, min_periods=3)
    .over("size_bucket")
...
# Run the Hierarchical + Vol-Adjusted BMA Engine
